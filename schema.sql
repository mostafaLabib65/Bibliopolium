...
begin
  update publishers
  set publishers.name         = name,
      publishers.address      = address,
      publishers.phone_number = phone_number
...
begin
  ...
  values (title, price, category, threshold, 0, NOW());
  ...
  values (book_id, edition, publisher_id, publishing_year, no_of_copies, NOW());
...
create procedure update_book(in book_id int, in title varchar(45), in price float, in category varchar(20),
                             in threshold int, in no_of_copies int)
begin
  update books
  set title        = title,
      price        = price,
...
create trigger request_an_order
  after update
  on books
  for each row
begin
  if NEW.no_of_copies < NEW.threshold then
    if not exists(select book_id from active_orders where active_orders.book_id = NEW.id) then
      insert into active_orders(book_id, quantity, created_at)
      values (NEW.id, 2 * (select threshold from books where books.id = NEW.id), now());
    end if;
  end if;
end;


create trigger discard_order
  after update
  on books
  for each row
begin
  ...
  select active_orders.id, active_orders.book_id, active_orders.quantity, active_orders.created_at
  from active_orders
  where active_orders.book_id = OLD.id into order_id, book_id, quantity, order_timestamp;
  if NEW.threshold < NEW.no_of_copies then
    if exists(select book_id from active_orders where active_orders.book_id = OLD.id) then
      insert into history_orders(id, book_id, quantity, order_created_at, status, created_at)
      values (order_id, book_id, quantity, order_timestamp, 'Discard', NOW());
    end if;
  end if;
end;

#---------------------------------------------book edition table-------------------------------------------------------------------------------
create procedure add_new_edition(in book_id int, in publisher_id int, in publishing_year int, in no_of_copies int,
                                 in edition int)
begin
  insert into book_editions(book_id, edition, publisher_id, publishing_year, no_of_copies, created_at)
  values (book_id, edition, publisher_id, publishing_year, no_of_copies, now());
...
begin
  ...
  where book_editions.publisher_id = publishers.id
    ...
    and book_editions.edition = edition;
end;


create procedure update_book_edition(in book_id int, in old_edition int, in new_edition int, in publisher_id int,
                                     in publishing_year int, in no_of_copies int)
begin
  update book_editions
  set book_editions.edition         = new_edition,
      book_editions.publisher_id    = publisher_id,
      book_editions.publishing_year = publishing_year,
      book_editions.no_of_copies    = no_of_copies
  where book_editions.edition = old_edition
    and book_editions.book_id = book_id;
end;
...
create trigger increase_no_of_copies
  after insert
  on book_editions
  for each row #done
begin
...
create trigger update_no_of_copies
  after update
  on book_editions
  for each row
begin
...
create trigger decrease_no_of_copies
  after delete
  on book_editions
  for each row
begin
...
create trigger delete_book_isbn
  after delete
  on book_editions
  for each row
begin
...
create trigger update_book_isbns_publisher_id
  after update
  on book_editions
  for each row
begin
  ...
  where book_isbns.book_id = OLD.book_id
    and book_isbns.publisher_id = OLD.publisher_id;
end;
#------------------------------------------------------book_isbns-----------------------------------------------------------------
...
begin
  select book_isbns.book_id, books.title, book_isbns.publisher_id, publishers.name, book_isbns.isbn
  from books,
       publishers,
       book_isbns
  where books.id = book_isbns.book_id
    and book_isbns.publisher_id = publishers.id;
end;
...
begin
  ...
  where books.id = book_isbns.book_id
    and book_isbns.publisher_id = publishers.id
    and book_isbns.book_id = book_id
    and book_isbns.publisher_id = publisher_id;
...
begin
  update book_isbns
  set book_isbns.isbn = isbn
  where book_isbns.publisher_id = publisher_id
    and book_isbns.book_id = book_id;
end;
...
begin
  ...
  from active_orders
  where active_orders.id = id into order_id, book_id, quantity, order_timestamp;
  insert into history_orders(id, book_id, quantity, order_created_at, status, created_at)
  values (id, book_id, quantity, order_timestamp, status, NOW());
  select max(edition) from book_editions where book_editions.book_id = book_id into max_edition;
  if status = 'Successful' then
    update book_editions
    set book_editions.no_of_copies = book_editions.no_of_copies + quantity
    where book_editions.book_id = book_id
      and book_editions.edition = max_edition;
  end if;
end;
...
create trigger delete_acrive_order
  after insert
  on history_orders
  for each row
begin
...
begin
  select book_id, title, sold_copies
  from statistics,
       books
  where statistics.book_id = books.id
...
begin
  select purchase_items_histories.book_id as id, sum(purchase_items_histories.quantity) * books.price as total_price
  from purchase_items_histories,
       books
  where purchase_items_histories.book_id = books.id
    and purchase_items_histories.created_at >=
        DATE_SUB((select max(purchase_items_histories.created_at) from purchase_items_histories), INTERVAL 1 MONTH)
...
begin
  select authors_books.book_id   as book_id,
  ...
  where authors_books.book_id = books.id
    and authors_books.author_id = authors.id;
end;
...
begin
  ...
  from authors_books,
       books,
       authors
  where authors_books.author_id = author_id
...
BEGIN
  ...
  ON DUPLICATE key update sold_copies = NEW.quantity + sold_copies,
...
create
  definer = root@localhost procedure UPDATE_USER(IN id_x bigint, IN user_name_x varchar(100), IN email_x varchar(100),
                                                 IN first_name_x varchar(100), IN last_name_x varchar(100),
                                                 IN shipping_address_x varchar(100), IN phone_number_x varchar(100),
                                                 IN passwd_x varchar(255), IN role_x tinyint)
BEGIN
...
create
  definer = root@localhost procedure add_item(IN user_id_x bigint, IN book_id_x int, IN edition_x int,
                                              IN quantity_x int)
BEGIN
...
create
  definer = root@localhost procedure checkout_cart(IN user_id_x bigint, IN credit_card_number varchar(32))
BEGIN
  ...
  CALL log_active_cart(user_id_x, "confirmed", TRUE);

end;

create
  definer = root@localhost function luhn(p_number varchar(31)) returns char
begin
...
create
  definer = root@localhost function luhn_check(p_number varchar(32)) returns tinyint(1)
begin
...
create
  definer = root@localhost procedure remove_item(IN user_id bigint, IN book_id_x int, IN edition_x int)
BEGIN
...
create
  definer = root@localhost procedure update_cart_count(IN cart_id_x int, IN increase int)
BEGIN
